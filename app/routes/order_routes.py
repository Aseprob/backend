"""
Endpoint with full crud operations for the order route
"""
...
def create_order():
    ...
    db = get_db()
    data = request.json
    new_order = Order(client_id=data['client_id'],
...
def order_operations(order_id):
    """
    Operations to interact with a specific order
    """
    db = get_db()
    ...
    if request.method == 'GET':
        ...
        data = request.json
        updated_order = Order(
            client_id=data['client_id'], product_id=data['product_id'], _id=ObjectId(order_id))
        db.orders.replace_one({'_id': ObjectId(order_id)},
