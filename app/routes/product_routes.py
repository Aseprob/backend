"""
Endpoint with full crud operations for the product route
"""
...
def get_products():
    ...
    """
    db = get_db()
    products = [Product.from_dict(product).to_dict()
...
def product_operations(product_id):
    """
    Operations to interact with a specific product
    """
    db = get_db()
    ...
    if request.method == 'GET':
        ...
        updated_product = Product(
            company=data['company'], description=data['description'], _id=ObjectId(product_id))
        db.products.replace_one(
            {'_id': ObjectId(product_id)}, updated_product.__dict__)
