...
def get_clients():
    db = get_db()
    clients = [Client.from_dict(client).to_dict() for client in db.clients.find()]
...
def client_operations(client_id):
    ...
    if request.method == 'GET':
        ...
        data = request.json
        updated_client = Client(email=data['email'], notes=data.get('notes'), _id=ObjectId(client_id))
        db.clients.replace_one({'_id': ObjectId(client_id)}, updated_client.__dict__)
